...
def main() -> int:
    ...
    for run_dir in run_dirs:
        ...
        model_pkl = run_dir / "model" / "model.pkl"
        preprocess_pkl = run_dir / "preprocess" / "preprocessor.pkl"

        missing = []
        for p, name in [
            (run_json, "run.json"),
            (metrics_json, "eval/metrics.json"),
            (inference_py, "inference/inference_example.py"),
            (model_pkl, "model/model.pkl"),
            (preprocess_pkl, "preprocess/preprocessor.pkl"),
        ]:
            if not p.exists():
                missing.append(name)

        if missing:
            results["file_anomalies"].append({"run_dir": rel_run, "missing": missing})
